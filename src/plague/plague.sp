...
public Action:MensajesSpawn(Handle:timer, any:client)
{
	if (IsClientInGame(client))
	{
		PrintToChat(client, "\x04[☣️ToxiC☣️] \x05%t", "Mata jugadores para conseguir creditos");
		PrintToChat(client, "\x04[☣️ToxiC☣️] \x05%t", "Escribe !premios para gastar tus creditos en premios");
	}
}
...
public PlayerDeathZM(any:attacker, any:client)
{
	...
	{
		PrintHintText(attacker, "[☣️ToxiC☣️]%t (+2)", "Tus creditos", g_iCredits[attacker]);
	}
	else
	{
		g_iCredits[attacker] = GetConVarInt(cvarCreditsMax);
		PrintToChat(attacker, "\x04[☣️ToxiC☣️] \x05%t", "Tus creditosM", g_iCredits[attacker]);
	}
...
public Event_hurtZM(any:attacker)
{
	...
	if(g_Movimientos[attacker] > GetConVarInt(disparos))
	{
		...
		if (g_iCredits[attacker] < GetConVarInt(cvarCreditsMax))
		{
			PrintHintText(attacker, "[☣️ToxiC☣️] %t (+1)", "Tus creditos", g_iCredits[attacker]);
		}
		else
		{
			g_iCredits[attacker] = GetConVarInt(cvarCreditsMax);
			PrintHintText(attacker, "[☣️ToxiC☣️] %t", "Tus creditosM", g_iCredits[attacker]);
		}
...
public Action:ZR_OnClientInfect(&client, &attacker, &bool:motherInfect, &bool:respawnOverride, &bool:respawn)
{
	...
	else if(inmunidad[client] > 0)
	{
		...
		PrintToChat(client, "\x04[☣️ToxiC☣️] \x05%t" ,"La armadura anti", inmunidad[client]);
		return Plugin_Handled;
	}
	return Plugin_Continue;
}
...
{
	PrintToChat(client, "\x04[☣️ToxiC☣️] \x05%t", "Tus creditos actuales son", g_iCredits[client]);
}
public Action:DOMenu(client,args)
{
	DID(client);
	PrintToChat(client, "\x04[☣️ToxiC☣️] \x05%t" ,"Tus creditos", g_iCredits[client]);
}
public Action:OnClientDisconnectZM(client)
{
	...
	if(Nemesis == client)
	{
		...
		Nemesis = 0;
		PrintToChatAll("\x04[☣️ToxiC☣️] \x05%t","El jugador NEMESIS se ha desconectado");
	}
	else if(Survivor == client)
	{
		ServerCommand("mp_restartgame 2");
		Survivor = 0;
		PrintToChatAll("\x04[☣️ToxiC☣️] \x05%t","El jugador SURVIVOR se ha desconectado");
	}
}
	...
	if (IsPlayerAlive(client))
	{
		if (g_iCredits[client] >= 7)
		{
			if (!ZR_IsClientZombie(client))
			{
				...
				g_iCredits[client] -= 7;
				PrintToChat(client, "\x04[☣️ToxiC☣️] \x05%t (-7)","Te has aplicado cirugia. Tus creditos", g_iCredits[client]);
			}
			else
			{
				PrintToChat(client, "\x04[☣️ToxiC☣️] \x05%t","Tienes que ser humano para poder comprar este premio");
			}
		}
		else
		{
			PrintToChat(client, "\x04[☣️ToxiC☣️] \x05%t 7)","Necesitas creditos", g_iCredits[client]);
		}
	}
	else
	{
		PrintToChat(client, "\x04[☣️ToxiC☣️] \x05%t","Pero si estas muerto");
	}
}
public Action:FijarCreditos(client, args)
{
	...
	{ 
		...
		{ 
			...
			PrintToChat(client, "\x04[☣️ToxiC☣️] \x05Puesto %i creditos en el jugador %N", amount, iClient);
		} 
	} 
	//}  
...
public Action:municionall(client, args)
{
	...
	PrintToChatAll("\x04[☣️ToxiC☣️] \x05%t", "Activada MUNICION INFINITA");
} 
...
public Action:OpcionNumero16c(Handle:timer, any:client)
{
	...
	{
		PrintToChat(client, "\x04[☣️ToxiC☣️] \x05%t","Ya no eres INVULNERABLE!");
		SetEntProp(client, Prop_Data, "m_takedamage", 2, 1);
...
public Action:DID(clientId) 
{
	...
	SetMenuTitle(menu, "☣️ToxiC☣️ Zombie Plague");
...
public Action:ResetAmmo2(Handle:timer)
{
	...
	PrintToChatAll("\x04[☣️ToxiC☣️] \x05%t","Has recibido 1 credito gratis! cada minuto recibiras 1 credito gratuito");
}
...
public DIDMenuHandler(Handle:menu, MenuAction:action, client, itemNum) 
{
	if ( action == MenuAction_Select ) 
	{
		...
		{
			PrintToChat(client,"\x04[☣️ToxiC☣️] \x05%t","Dispara a los zombies o infecta humanos para ganar creditos");
			PrintToChat(client, "\x04[☣️ToxiC☣️] \x05%t","creado para SourceMod, encuentralo en www.servers-cfg.foroactivo.com", VERSION2);
			DID(client);
		}
			...
			if (g_iCredits[client] >= 7)
			{
				if (IsPlayerAlive(client))
				{
					if (!ZR_IsClientZombie(client))
					{
						...
						{
							PrintToChat(client,"\x04[☣️ToxiC☣️] \x05%t","No puedes comprar cosas siendo un ser especial");
							return;
						}
						SetEntityHealth(client, 100);
						g_iCredits[client] -= 7;
						...
						//PrintToChatAll("\x04[☣️ToxiC☣️] \x05El jugador\x03 %s \x05se ha curado!", nombre);
						PrintToChat(client, "\x04[☣️ToxiC☣️] \x05%t (-7)","Te has aplicado cirugia. Tus creditos", g_iCredits[client]);
					}
					else
					{
				...
				{
					PrintToChat(client, "\x04[☣️ToxiC☣️] \x05%t","Tienes que estar vivo para poder comprar premios");
				}
			}
			else
			{
		...
		else if ( strcmp(info,"option3") == 0 ) 
		{
			if (g_iCredits[client] >= 8)
			{
				if (IsPlayerAlive(client))
				{
					if (!ZR_IsClientZombie(client))
					{
						...
						g_iCredits[client] -= 8;
						//EmitSoundToAll("medicsound/medic.wav");
						PrintToChat(client, "\x04[☣️ToxiC☣️] \x05%t (-8)","ahora tienes 250 HP. Tus creditos", g_iCredits[client]);
					}
					else
					{
						PrintToChat(client, "\x04[☣️ToxiC☣️] \x05%t","Tienes que ser humano para poder comprar este premio");
			...
			{
				PrintToChat(client, "\x04[☣️ToxiC☣️] \x05%t 8)","Necesitas creditos", g_iCredits[client]);
			}
			DID(client);
		...
		else if ( strcmp(info,"option4") == 0 ) 
		{
			...
			if (g_iCredits[client] >= 9)
			{
				if (GetClientTeam(client) != 1 && IsPlayerAlive(client))
				{
					...
					g_iCredits[client] -= 9;
					PrintToChat(client, "\x04[☣️ToxiC☣️] \x05%t (-9)","Has comprado una municion para LANZALLAMAS! Se usa apretando boton derecho del raton a cuxi.  Tus creditos", g_iCredits[client]);
				}
				else
				{
					PrintToChat(client, "\x04[☣️ToxiC☣️] \x05%t","Tienes que estar vivo para poder comprar premios");
			...
			{
				PrintToChat(client, "\x04[☣️ToxiC☣️] \x05%t 9)","Necesitas creditos", g_iCredits[client]);
			}
			DID(client);
		...
		else if ( strcmp(info,"option5") == 0 ) 
		{
			...
			if (g_iCredits[client] >= 9)
			{
				if (GetClientTeam(client) != 1 && IsPlayerAlive(client))
				{
					...
					g_iCredits[client] -= 9;
					PrintToChat(client, "\x04[☣️ToxiC☣️] \x05%t (-9)","Ahora tienes DOBLE SALTO y puedes saltar en el aire! Tus creditos", g_iCredits[client]);
				}
				else
				{
					PrintToChat(client, "\x04[☣️ToxiC☣️] \x05%t","Tienes que estar vivo para poder comprar premios");
		...
		else if ( strcmp(info,"option6") == 0 ) 
		{
			...
			if (g_iCredits[client] >= 14)
			{
				if (GetClientTeam(client) != 1 && IsPlayerAlive(client))
				{
					...
					PrintToChat(client, "\x04[☣️ToxiC☣️] \x05%t (-14)","Has comprado un cuchillo arrojadizo. Tus creditos", g_iCredits[client]);
				}
				else
				{
					PrintToChat(client, "\x04[☣️ToxiC☣️] \x05%t","Tienes que estar vivo para poder comprar premios");
			...
			{
				PrintToChat(client, "\x04[☣️ToxiC☣️] \x05%t 14)","Necesitas creditos", g_iCredits[client]);
			}
			DID(client);
		...
		else if ( strcmp(info,"option7") == 0 ) 
		{
			if (g_iCredits[client] >= 15)
			{
				if (IsPlayerAlive(client))
				{
					if (!ZR_IsClientZombie(client))
					{
						...
						PrintToChat(client, "\x04[☣️ToxiC☣️] \x05%t (-15)","Has comprado un escudo anti-infeccion que resiste 5 golpes! Tus creditos", g_iCredits[client]);
					}
					else
					{
						PrintToChat(client, "\x04[☣️ToxiC☣️] \x05%t","Tienes que ser humano para poder comprar este premio");
			...
			{
				PrintToChat(client, "\x04[☣️ToxiC☣️] \x05%t 15)","Necesitas creditos", g_iCredits[client]);
			}
			DID(client);
		...
		else if ( strcmp(info,"option8") == 0 ) 
		{
			...
			if (g_iCredits[client] >= 15)
			{
				if (IsPlayerAlive(client))
				{
					...
					g_iCredits[client] -= 15;
					PrintToChat(client, "\x04[☣️ToxiC☣️] \x05%t (-15)","Ahora tienes mas velocidad! Tus creditos", g_iCredits[client]);
				}
				else
				{
					PrintToChat(client, "\x04[☣️ToxiC☣️] \x05%t","Tienes que estar vivo para poder comprar premios");
		...
		else if ( strcmp(info,"option9") == 0 ) 
		{
			...
			if (g_iCredits[client] >= 15)
			{
				if (IsPlayerAlive(client))
				{
					...
					g_iCredits[client] -= 15;
					PrintToChat(client, "\x04[☣️ToxiC☣️] \x05%t (-15)","Ahora eres un Barril! Tus creditos", g_iCredits[client]);
				}
				else
				{
					PrintToChat(client, "\x04[☣️ToxiC☣️] \x05%t","Tienes que estar vivo para poder comprar premios");
		...
		else if ( strcmp(info,"option10") == 0 ) 
		{
			...
			if (g_iCredits[client] >= 16)
			{
				if (IsPlayerAlive(client))
				{
					if (!ZR_IsClientZombie(client))
					{
						...
						PrintToChat(client, "\x04[☣️ToxiC☣️] \x05%t (-16)","Ahora tienes municion infinita! Tus creditos", g_iCredits[client]);
					}
					else
					{
						PrintToChat(client, "\x04[☣️ToxiC☣️] \x05%t","Tienes que ser humano para poder comprar este premio");
			...
			{
				PrintToChat(client, "\x04[☣️ToxiC☣️] \x05%t 16)","Necesitas creditos", g_iCredits[client]);
			}
			DID(client);
		...
		else if ( strcmp(info,"option11") == 0 ) 
		{
			...
			if (g_iCredits[client] >= 18)
			{
				if (IsPlayerAlive(client))
				{
					if (ZR_IsClientZombie(client))
					{
						...
						if(NoCurar)
						{
							PrintToChat(client,"\x04[☣️ToxiC☣️] \x05%t","No puedes curarte al empezar la ronda hasta que no pase 1 minuto");
							return;
						}
						if(!infectado[client])
						{
							PrintToChat(client,"\x04[☣️ToxiC☣️] \x05No puedes curarte siendo mother zombie o no habiendo infectado a alguien primero");
							return;
						}
						...
						PrintToChat(client, "\x04[☣️ToxiC☣️] \x05%t (-18)","Te has curado y vuelves a ser humano! Tus creditos", g_iCredits[client]);
					}
					else
					{
						PrintToChat(client, "\x04[☣️ToxiC☣️] \x05%t","Tienes que ser zombie para poder comprar este premio");
					}
				}
				else
				{
			...
			{
				PrintToChat(client, "\x04[☣️ToxiC☣️] \x05%t 18)","Necesitas creditos", g_iCredits[client]);
			}
			DID(client);
		...
		else if ( strcmp(info,"option12") == 0 ) 
		{
			if (g_iCredits[client] >= 20)
			{
				if (IsPlayerAlive(client))
				{
					...
					PrintToChat(client, "\x04[☣️ToxiC☣️] \x05%t (-20)","Ahora eres invisible! Tus creditos", g_iCredits[client]);
				}
				else
				{
					PrintToChat(client, "\x04[☣️ToxiC☣️] \x05%t","Tienes que estar vivo para poder comprar premios");
			...
			{
				PrintToChat(client, "\x04[☣️ToxiC☣️] \x05%t 20)","Necesitas creditos", g_iCredits[client]);
			}
			DID(client);
		...
		else if ( strcmp(info,"option13") == 0 ) 
		{
			...
			if (g_iCredits[client] >= 50)
			{
				if (IsPlayerAlive(client))
				{
					...
					PrintToChat(client, "\x04[☣️ToxiC☣️] \x05%t (-50)","Ahora eres INVULNERABLE durante 5 segundos! Tus creditos", g_iCredits[client]);
				}
				else
				{
					PrintToChat(client, "\x04[☣️ToxiC☣️] \x05%t","Tienes que estar vivo para poder comprar premios");
			...
			{
				PrintToChat(client, "\x04[☣️ToxiC☣️] \x05%t 50)","Necesitas creditos", g_iCredits[client]);
			}
			DID(client);
			...
			if (g_iCredits[client] >= 50)
			{
				if (IsPlayerAlive(client))
				{
					if (!ZR_IsClientZombie(client))
					{
						...
						PrintToChat(client, "\x04[☣️ToxiC☣️] \x05%t (-50)","Ahora tienes una Deagle DORADA ARDIENTE que produce mas damage y quema! Tus creditos", g_iCredits[client]);
					}
					else
					{
						PrintToChat(client, "\x04[☣️ToxiC☣️] \x05%t","Tienes que ser humano para poder comprar este premio");
		...
		else if ( strcmp(info,"option15") == 0 ) 
		{
			...
			if (g_iCredits[client] >= 60)
			{
				if (IsPlayerAlive(client))
				{
					if (ZR_IsClientZombie(client))
					{
						...
						PrintToChat(client, "\x04[☣️ToxiC☣️] \x05%t (-60)","Has comprado una BOMBA DE INFECCION! Tus creditos", g_iCredits[client]);
					}
					else
					{
						PrintToChat(client, "\x04[☣️ToxiC☣️] \x05%t","Tienes que ser zombie para poder comprar este premio");
			...
			{
				PrintToChat(client, "\x04[☣️ToxiC☣️] \x05%t 60)","Necesitas creditos", g_iCredits[client]);
			}
			DID(client);
			...
			if (g_iCredits[client] >= 60)
			{
				if (IsPlayerAlive(client))
				{
					if (!ZR_IsClientZombie(client))
					{
						...
						nade_count[client] += 1;
						GivePlayerItem(client, "weapon_hegrenade");
						PrintToChat(client, "\x04[☣️ToxiC☣️] \x05%t (-60)","Has comprado una BOMBA DE ANTIDOTO! Tus creditos", g_iCredits[client]);
					}
					else
					{
						PrintToChat(client, "\x04[☣️ToxiC☣️] \x05%t","Tienes que ser humano para poder comprar este premio");
		...
		else if ( strcmp(info,"option17") == 0 ) 
		{
			...
			if (g_iCredits[client] >= 60)
			{
				if (IsPlayerAlive(client))
				{
					...
					g_Bomba[client] = true;
					PrintToChat(client, "\x04[☣️ToxiC☣️] \x05%t (-60)","Ahora eres una KAMIKAZE EXPLOSIVO! escribe !detonar para estallar! Tus creditos", g_iCredits[client]);
				}
				else
				{
					PrintToChat(client, "\x04[☣️ToxiC☣️] \x05%t","Tienes que estar vivo para poder comprar premios");
			...
			if (g_iCredits[client] >= 60)
			{
				if (IsPlayerAlive(client))
				{
					if (!ZR_IsClientZombie(client))
					{
						...
						PrintToChat(client, "\x04[☣️ToxiC☣️] \x05%t (-60)","Ahora tienes una MP5 NAVY DORADA ARDIENTE que produce mas damage y quema! Tus creditos", g_iCredits[client]);
					}
					else
					{
						PrintToChat(client, "\x04[☣️ToxiC☣️] \x05%t","Tienes que ser humano para poder comprar este premio");
...
Ronda_Plague()
{
	...
	PrintToChatAll("\x04[☣️ToxiC☣️] \x05%t","RONDA Plague!");
...
Ronda_Survivor()
{
	...
	PrintToChatAll("\x04[☣️ToxiC☣️] \x05%t","RONDA SURVIVOR!");
...
Ronda_Nemesis()
{
	...
	PrintToChatAll("\x04[☣️ToxiC☣️] \x05%t","RONDA NEMESIS!");
...
public Action:NemesisA(client, args)
{
	...
	{
		PrintToChat(client, "\x04[☣️ToxiC☣️] \x05Objetivo no encontrado");
...
{ 
	...
	decl String:nombre[32];
	GetClientName(client, nombre, sizeof(nombre));
	PrintToChat(client, "\x04[☣️ToxiC☣️] \x05%t","El admin ha borrado todos los props que se habian puesto", nombre); // english
	return Plugin_Handled;
}
...
public Action:Dar(client, args)
{
	...
	{
		PrintToChat(client, "\x04[☣️ToxiC☣️] \x05%t","Tu no tienes tantos creditos!");
		return Plugin_Handled; // Target not found...
	}
	if(amount <= 0)
	{
		PrintToChat(client, "\x04[☣️ToxiC☣️] \x05%t","No puedes dar menos de 0 creditos!");
		return Plugin_Handled; // Target not found...
	}
	new target;
	if((target = FindTarget(client, arg, false, false)) == -1)
	{
		PrintToChat(client, "\x04[☣️ToxiC☣️] \x05Objetivo no encontrado (not found)");
		return Plugin_Handled; // Target not found...
	}
	//    SetEntProp(target, Prop_Data, "m_iDeaths", amount);
	...
	PrintToChat(client, "\x04[☣️ToxiC☣️] \x05%t","Entregados", amount, nombre2);
	PrintToChat(target, "\x04[☣️ToxiC☣️] \x05%t","Te ha Entregado", amount, nombre);
	return Plugin_Continue;
} 
public Action:Darme(client, args)
{
	...
	g_iCredits[client] += amount;
	PrintToChat(client, "\x04[☣️ToxiC☣️] \x05Te has dado %i creditos", amount);
	return Plugin_Continue;
}  
public Action:VerCreditosClient(client, args)
{
	...
	{ 
		...
		{ 
			//g_iCredits[iClient] = amount;
			PrintToChat(client, "\x04[☣️ToxiC☣️] \x05%N Creditos: %i", iClient, g_iCredits[iClient]);
			//PrintToChat(client, "\x04[☣️ToxiC☣️] \x05Puesto %i creditos en el jugador %N", amount, iClient);
		} 
	}   
	return Plugin_Continue;
